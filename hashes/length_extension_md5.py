...
def length_extension(hsh: bytes, msg: bytes, ext: bytes, head: int = 0) -> tuple[bytes, bytes]:
    """length_extension(hsh, msg, ext, head=0): given hsh = md5(pre+msg),
    returns msg2 = msg+pad+ext and hsh2 such that hsh2 = md5(pre+msg2) 
    """
    mlen = len(msg) + head
    pad  = padding(mlen*8)
    bits = (mlen + len(pad))*8

    h = md5(state=hsh, count=bits)
    h.update(ext)
    return (msg + pad + ext, h.digest())
...
def test(input=b''):
    ...
    """
    import hashlib
    print('our impl:', md5(input).hexdigest())
    print('standard:', hashlib.md5(input).hexdigest())

    pre = b'secret'
    msg = b'some msg'
    ext = b'some ext'
    hsh = hashlib.md5(pre + msg).digest()
    msg2, hsh2 = length_extension(hsh, msg, ext, len(pre))
    print('prediction:', hsh2.hex())
    print('actual hsh:', hashlib.md5(pre + msg2).hexdigest())
